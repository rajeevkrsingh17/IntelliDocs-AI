...
This document details the complete technology stack used in **IntelliDocs-AI** for multi-format document ingestion, semantic search, generative synthesis, and user interfaces.
...
| **Document Extraction** | PyMuPDF (`fitz`), python-docx, python-pptx | 1.23+ / 1.2.0 / 1.0.2 | Extract text and structures from PDF, DOCX, PPTX, MD, and TXT files. Converts PDF tables to Markdown tables via `find_tables()`. |
...
1. **Frontend Hosting (React SPA):**
   - Deployed on **Vercel** (`https://intellidocs-ai-tau.vercel.app`)
   - Configured with rewrite rules in `vercel.json` to handle client-side routing.
   
2. **Backend Hosting (FastAPI REST API):**
   - Deployed on **Render** (`https://intellidocs-api-yedx.onrender.com`)
